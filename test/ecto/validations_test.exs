defmodule Ecto.ValidationsTest do
  ...
  test Email do
    assert true == V.valid?(Email[allow_nil: true], nil)
    assert true == V.valid? Email[], "gary@google.com"
    assert true == V.valid? Email[], "gary+snail@google.co.uk"
    assert V.valid?(Email[], "gary2garythesnail.com") == :bad_format
    assert true == V.valid? Email[check_domain: true], "gary@google.com"
    assert V.valid?(Email[check_domain: true], "gary@gewgla.com") == :bad_domain
  end

  test Phone do
    assert V.valid?(Phone[allow_nil: true], nil) == true
    assert V.valid?(Phone[], nil) == :bad_phone
  ...
  test Date do
    assert V.valid?(Date[], nil) == :bad_date
    assert V.valid?(Date[allow_nil: true], nil) == true
  ...
  test DateTime do
    assert V.valid?(DateTime[], nil) == :bad_datetime
    assert V.valid?(DateTime[allow_nil: true], nil) == true
